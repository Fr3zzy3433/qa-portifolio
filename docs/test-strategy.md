...
- **Authentication Testing:** Validation of `x-api-key` enforcement and unauthorized responses.
...
- Known behavioral findings may assert the current observed behavior for regression visibility, while the documentation preserves the original hypothesis and its limitation.
...
- **No authoritative business requirements:** For exploratory edge cases, the portfolio can state a hypothesis and observed behavior but cannot confirm a business defect without an explicit contract.
