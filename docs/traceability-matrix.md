...
| **CT001.003** | GET | `CT001.003 - Retrieve existing user without x-api-key (Known Finding)` | ReqRes Published Authentication Contract vs. Observed Behavior | FIND-004 | Automated (Known Finding) |
...
| **CT002.003** | POST | `CT002.003 - Create user with empty body payload (Validation Finding)` | Test Hypothesis / Input Validation Expectation | FIND-001 | Automated (Finding) |
...
| **CT003.003** | PUT | `CT003.003 - Update user with non-existent ID (Validation Finding)` | Test Hypothesis / API Domain Expectation | FIND-002 | Automated (Finding) |
| **CT003.004** | PUT | `CT003.004 - Update existing user with empty body payload (Validation Finding)` | Test Hypothesis / Input Validation Expectation | FIND-003 | Automated (Finding) |
